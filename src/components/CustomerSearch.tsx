...
import { Input, AutoComplete, Button, Spin, Empty, Card, Row, Col, Typography, Divider } from 'antd';
import { UserOutlined, SearchOutlined, CloseCircleOutlined, HistoryOutlined } from '@ant-design/icons';
...
const CustomerSearch: React.FC<CustomerSearchProps> = ({ onSelectCustomer, selectedCustomer }) => {
  const [searchText, setSearchText] = useState('');
  const [options, setOptions] = useState<{ value: string; label: React.ReactNode; customer: Customer }[]>([]);
  ...
  const searchCustomers = async (value: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('kv_customers')
        .select('*')
        .or(`name.ilike.%${value}%,code.ilike.%${value}%,contact_number.ilike.%${value}%`)
        .limit(10);
      ...
      if (data && data.length > 0) {
        const formattedOptions = data.map(customer => ({
          ...
          label: (
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span><UserOutlined /> {customer.name}</span>
        ...
        if (allCustomers) {
          const filteredCustomers = allCustomers.filter(customer => 
            removeAccents(customer.name?.toLowerCase() || '').includes(normalizedValue) ||
            removeAccents(customer.code?.toLowerCase() || '').includes(normalizedValue) ||
    ...
    return (
      <div style={{ width: '100%' }}>
        <Card 
          ...
          extra={<Button type="text" icon={<CloseCircleOutlined />} onClick={handleClear} />}
        ...
        >
          <Row gutter={[8, 8]}>
            ...
            <Col span={12}>
              <Text type="secondary">SĐT: {selectedCustomer.contact_number}</Text>
            </Col>
            {selectedCustomer.location_name && (
              <Col span={24}>
                <Text type="secondary">Địa chỉ: {selectedCustomer.location_name}</Text>
              </Col>
            )}
          </Row>
          ...
          {customerInvoices.length > 0 && (
            <>
              <Divider plain><Button type="link" icon={<HistoryOutlined />} onClick={toggleHistory} size="small">
                ...
                <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
                  {customerInvoices.map(invoice => (
                    <Card 
                      ...
                      title={`HD-${invoice.code} (${new Date(invoice.purchase_date).toLocaleDateString('vi-VN')})`}
                      type="inner"
                    >
                      <div>
                        <Text>Tổng: {invoice.total?.toLocaleString('vi-VN')} VND</Text>
                      </div>
                      <div>
                        <Text type="secondary">Số mặt hàng: {invoice.kiotviet_invoice_details?.length || 0}</Text>
                      </div>
                    </Card>
                  ))}
                </div>
  ...
  return (
    <div style={{ width: '100%' }}>
      <AutoComplete
        ...
        value={searchText}
        style={{ width: '100%' }}
        notFoundContent={loading ? <Spin size="small" /> : searchText.length >= 3 ? <Empty description="Không tìm thấy" /> : "Nhập tối thiểu 3 ký tự"}
        placeholder="Tìm khách hàng theo tên, mã, số điện thoại..."
      >
        <Input.Search size="large" loading={loading} prefix={<UserOutlined />} enterButton />
      </AutoComplete>
    </div>
  );
};
