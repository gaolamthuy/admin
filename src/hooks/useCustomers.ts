...
export const useCustomers = (searchTerm?: string) => {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Customer[]> => {
      ...
      const baseQuery = supabase.from('v_customers_admin').select(
        `
          id,
          kiotviet_id,
          code,
      ...
      const sortedData = (data || []).sort((a, b) => {
        ...
        return compareDates(
          b.latest_invoice_datetime,
          a.latest_invoice_datetime
        );
      });
...
export const useCustomer = (id: string | number) => {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<
      (Customer & { recent_invoices?: any }) | null
    > => {
      if (!session) {
        throw new Error('Not authenticated');
      }
