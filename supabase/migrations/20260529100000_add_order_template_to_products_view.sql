-- Migration: Add order_template to v_products_admin view
-- Update view to include order_template field from kv_products table
-- NOTE: Must DROP and recreate because CREATE OR REPLACE VIEW cannot add columns
-- in the middle of the SELECT list (changes column ordering which Postgres rejects)

DROP VIEW IF EXISTS v_products_admin;
...
SELECT
    ...
    CASE
        ...
        THEN ceiling((lp.latest_total_cost_per_unit + COALESCE(p.glt_baseprice_markup, 0)::numeric) / COALESCE(p.glt_baseprice_round_step, 1000)::numeric) * COALESCE(p.glt_baseprice_round_step, 1000)::numeric
        ELSE NULL
    ...
    CASE
        WHEN lp.latest_total_cost_per_unit IS NOT NULL OR inv.cost IS NOT NULL
...
COMMENT ON VIEW v_products_admin IS 'Admin products view with order_template field for display in product list';